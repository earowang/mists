...
The `na_rle()` gives a sparser representation for indexing runs of missings, a special type of run length encoding (`rle`).
...
It returns a named list of `lengths` and `indices` (the starting indices) of `NA` runs. Set operations can be applied: (1) `intersect(x, y)`, (2) `union(x, y)`, (3) `setdiff(x, y)`, along with other math operations.
